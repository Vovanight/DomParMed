...
$this->app->teaser_magazin->setView($this);
...
if (!$this->app->jbcache->start($this->params->get('config.lastmodified'))) {
    ...
    // category render
    if (isset($category)) {
        echo $this->app->teaser_magazin->render($currentView, $category);
    }

    // alphaindex render
    if ($this->params->get('template.show_alpha_index', 0)) {
        echo $this->app->teaser_magazin->render('alphaindex', $this->alpha_index);
    }
    ...
    if (isset($category)) {
        ...
        if ($this->params->get('template.subcategory_show', 1) && count($categories)) {
            echo $this->app->teaser_magazin->render('subcategories', $categories);
        }
    }
    ...
    if ($this->params->get('config.items_show', 1) && count($this->items)) {
        ...
        echo $this->app->teaser_magazin->render('teaser_magazin', $this->items);

    } else {
        echo $this->app->teaser_magazin->render('items_empty', $category);
    }

    // pagination render
    if ($this->params->get('template.item_pagination', 1)) {
        echo $this->app->teaser_magazin->render('pagination', $this->pagination, array('link' => $this->pagination_link));
    }
